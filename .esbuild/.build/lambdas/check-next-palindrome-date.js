...
// lambdas/check-next-palindrome-date.ts
var check_next_palindrome_date_exports = {};
__export(check_next_palindrome_date_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(check_next_palindrome_date_exports);
var import_deep_object_js = __toESM(require_deep_object_js());
function addDay(date) {
  return new Date(date.getTime() + 24 * 60 * 60 * 1e3);
}
function checkIsPalindromeDate(date) {
  const dateAsString = date.toISOString();
  const dateAsStringReversed = dateAsString.split("-").reverse().join("-");
  console.log(`Checking if ${dateAsString} is a palindrome...`);
  console.log(`Reversed date: ${dateAsStringReversed}`);
...
var handler = async (event) => {
  ...
  console.log(`Checking if ${date} is a palindrome date...`);
  while (!checkIsPalindromeDate(date)) {
    console.log(`Date ${date} is not a palindrome date, adding a day...`);
    date = addDay(date);
  }
  return {
    statusCode: 200,
...
//# sourceMappingURL=check-next-palindrome-date.js.map
